...
if (/Unsere Farben\. Unser Stolz\./.test(read('src/LiveDashboard.tsx'))) pass('Willkommens-Slogan ist eingebaut.'); else fail('Willkommens-Slogan fehlt.');
if (/is-matchday/.test(read('src/LiveDashboard.tsx')) && /v183-matchday-title/.test(read('src/LiveDashboard.tsx')) && />MATCHDAY</.test(read('src/LiveDashboard.tsx')) && /<small>TSU Ainet<\/small>/.test(read('src/LiveDashboard.tsx'))) pass('Matchday-Hero ist eingebaut.'); else fail('Matchday-Hero fehlt.');
