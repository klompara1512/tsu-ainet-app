                  ...
                  <div className="team-player-head-sponsor">
                    ...
                    {selectedPlayer.headSponsorWebsite ? (
                      <a href={selectedPlayer.headSponsorWebsite} target="_blank" rel="noreferrer">
                        {selectedPlayer.headSponsorImageUrl && <AutoFitLogo src={selectedPlayer.headSponsorImageUrl} alt={selectedPlayer.headSponsorText || "Kopfsponsor"} className="team-player-head-sponsor-logo" trim={false} />}
  ...
  return (
    <section className="teams-page">
      ...
      {!isLoadingTeams && teams.length > 0 && !teamsError && <div className="teams-grid">{teams.map((team) => {
        ...
        return <button key={team.id} type="button" className={`team-card team-card-${teamKeyFromName(team.name)} ${team.imageUrl ? "has-team-photo" : ""}`} onClick={() => openTeam(team.id)} style={team.imageUrl ? { backgroundImage: `linear-gradient(90deg, rgba(6,13,27,.94), rgba(6,13,27,.58)), url(${team.imageUrl})` } : undefined}><span className="team-card-photo">{team.imageUrl ? <img src={team.imageUrl} alt="" aria-hidden="true" /> : <Icon name={teamIcon(team.name)} />}</span><span className="team-info"><small>{leagueLabel(team)}</small><strong>{team.name}</strong><em>{next ? `Nächstes Spiel ${formatDate(next.kickoffAt)}` : "Keine kommenden Spiele"}</em></span><span className="team-card-position">{position ? `#${position}` : "–"}<small>Platz</small></span><span className="team-arrow">›</span></button>;
      })}</div>}
